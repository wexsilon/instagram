            ...
            if ($_FILES['photo']['size'] > 0) {
                if (getimagesize($_FILES["photo"]["tmp_name"]) !== false) {
                    ...
                    if (in_array($imageFileType, array('jpg', 'png', 'jpeg', 'gif'))) {
                        ...
                        $sql = "INSERT INTO posts (username, photo, caption) VALUES (?, ?, ?)";
                        $stmt = $pdoObj->prepare($sql);
                        $stmt->execute([$username, $outf, $_POST['caption']]);
                        
                        $sql = "UPDATE users SET post_count = post_count + 1 WHERE username = ?";
                        $stmt = $pdoObj->prepare($sql);
    ...
    <body>
        ...
        <main role="main" class="container-fluid">
            <div class='row justify-content-center'>         
                ...
                <div class='card col-12 col-sm-8 col-md-5 p-4 text-center p-4 pt-5' style="margin-top:5rem">
                    <p><img src="static/img/logo.png" alt="logo" style="width:180px"></p>  
                    <h5>Create New Post</h5>
                    <p class='text-muted mb-5'>Upload an image and add a caption.</p>

                    <form method="post" enctype="multipart/form-data">
                        <div class="input-group d-flex justify-content-center mb-3">
                            <div class="custom-file col-11">
                                <input id="file-upload" type="file" name="photo">
                                <label id="showfilename" class="custom-file-label d-inline" for="file-upload" style="background-color: #fafafa;">Choose file</label>
                            </div>
                        </div>
                        <input type="text" name="caption" value="" class="post-create-form" placeholder="Add a caption...">
                        <input type="submit" class="btn btn-primary col-6 mt-2 p-1" value="Upload" style="font-size: 0.9rem; font-weight: 600; background-color:#0095f6;" />
                    </form>
                </div>
            ...
            document.getElementById('file-upload').addEventListener(
                ...
                function(e) {
                    if (e.target.files[0]) {
                        ...
                        document.getElementById('showfilename').innerHTML = e.target.files[0].name;
                    } else {
                        document.getElementById('showfilename').innerHTML = 'Choose file';
                    }
                }
            );
        </script>
