...
$sql = "SELECT fullname, username, post_count, follower_count, following_count, bio, photo, website, phone FROM users WHERE id = ?";
        ...
        <main role="main" class="container-fluid">
            <div class='row justify-content-center'>         
                ...
                <div class='container col-12 col-lg-9 justify-content-start' style="padding: 0; margin: 0;">
                    <div class='row' style="margin-top: 5rem;">
                        <div class="media pb-5 border-bottom">
                            ...
                            <div class="media-body m-0" style="font-size: 1rem;">
                                ...
                                <div><b><?php echo $fullname ?></b></div>
                                <div><?php echo $bio ?></div>
                                <div> <a href='<?php echo $website ?>'><b><?php echo $website ?></b></a></div>
                            </div>  
                        </div>
                    </div>
                    <div class='row col-12 justify-content-center' style="font-size: 0.85rem;">
                        <a class="mr-5 ml-5 ml-md-0 text-secondary" href="profile.php" style="padding-top:1rem">       
                        ...
                        <a class="mr-5 text-dark border-top border-dark" href="saved.php" style="padding-top:1rem">       
                    ...
                    <small class="row text-muted mt-5 ml-4">Only you can see what you've saved</small><br>
                    <div class='row m-0'>
                        <?php
                            $sqlQuerySave = "SELECT pid FROM saves WHERE username = ?";
                            $stmtSave = $pdoObj->prepare($sqlQuerySave);
                            $stmtSave->execute([$username]);
                            if ($stmtSave->rowCount() > 0) {
                                while ($save = $stmtSave->fetch(PDO::FETCH_ASSOC)){
                                    $spid = $save['pid'];
                                    $sqlQueryPost = "SELECT photo, like_count, comment_count FROM posts WHERE id = ?";
                                    $stmtPost = $pdoObj->prepare($sqlQueryPost);
                                    $stmtPost->execute([$spid]);
                                    $post = $stmtPost->fetch(PDO::FETCH_ASSOC);
                                    $spphoto = $post['photo'];
                                    $splc = intval($post['like_count']);
                                    $spcc = intval($post['comment_count']);
                                    
                        ?>
                                     
                        <div class="modal fade share-modal" id="share-modal-<?php echo $spid; ?>" tabindex="-1" role="dialog" aria-labelledby="share-modalLabel" aria-hidden="true">
                            <div class="modal-dialog  modal-dialog-centered" style="max-width: 40%; height: 50vh !important;" role="document">
                                <div class="modal-content" style="border-radius: .75rem !important;">
                                    ...
                                    <form onsubmit="event.preventDefault()" class="search-form p-3 col-12" autocomplete="off" spellcheck="false">
                                        <label for='share-label-<?php echo $spid; ?>' class="col-1 p-0"><b>To:</b> </label>
                                        <input class="share-input col-8" id="share-input-<?php echo $spid; ?>" type="text" placeholder="Search..." aria-label="Search" onfocus="clearField(this)" autofocus style="border:none; outline:none;">
                                    </form>
                                    <div class='modal-footer justify-content-start' style="height: 30vh; overflow-y: auto;">
                                        <div class='share-search-results' id='share-search-results-<?php echo $spid; ?>' style=" width:100%;">
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <span class='post-overlay-container col-4 p-0 p-md-2'>
                            <img class="btn col-12 p-1" src="<?php echo $spphoto; ?>" style="height: 100%;">
                            <div class="post-overlay all-comments" id='all-comments-<?php echo $spid; ?>'>
                                <div class="overlay-text">
                                    <i class='fa fa-heart col-2'></i>  <?php echo $splc; ?>       
                                    <span class='d-none d-md-inline col-md-3'></span>
                                    <i class='fa fa-comment'></i> <?php echo $spcc; ?>
                                </div>
                            </div>
                        </span>
                        <?php
                        ...
                        ?>
                        <div class='no-saved-post row col-12 mt-3 justify-content-center'>
