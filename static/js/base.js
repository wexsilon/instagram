...
$(document.body).on('click','.all-comments', function(){
    ...
    $.get(`/instagram/p.php?pid=${post_id}`, function(response) {
...
$(document.body).on('click','.follow-btn', function() {
    ...
    user_username = new URL(window.location.href).searchParams.get('username');
    console.log('follow request sent @ /follow/');

    $.post('follow.php',{'username':user_username}, function(response){
